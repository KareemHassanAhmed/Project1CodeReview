...
async function populateEmployeeTable()
{
    ...
    let response = await fetch(`http://localhost:7070//reimbursement?employeeId=${JSON.parse(localStorage.getItem("data"))[0].eid}`);
...
{
    let response = await fetch(`http://localhost:7070/expense-category/${cid}`);
    let info = await response.json();
    return info;
}
...
{
    let response = await fetch(`http://localhost:7070/reimbursement/${rid}`);
    let info = await response.json();
    return info;
}
...
{
    let response = await fetch(`http://localhost:7070/employee/${eid}`);
    let info = await response.json();
    return info;
}
...
async function populateBullets()
{
    ...
    let response = await fetch(`http://localhost:7070/expense-categories`);
    let info = await response.json();
...
async function uploadNewReimbursement()
{
    ...
    let response = await fetch("http://localhost:7070/reimbursement", requestOptions)
