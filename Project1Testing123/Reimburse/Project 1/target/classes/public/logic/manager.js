...
async function populateManagerTable()
{
    ...
    let response = await fetch(`http://localhost:7070/reimbursement?managerId=${JSON.parse(localStorage.getItem("data"))[0].mgid}`);
...
async function statusUpdate(status)
{
    ...
    await fetch("http://localhost:7070/reimbursement", requestOptions)
